defmodule SymphonyElixir.Config do
  ...
  def agent_kind do
    get_in(validated_workflow_options(), [:agent, :kind]) || "codex"
  end
  ...
  defp require_agent_config do
    case agent_kind() do
      ...
      kind when kind in ["ghcopilot", "claude", "gemini", "echo"] ->
        :ok

      other ->
        {:error, {:unsupported_agent_kind, other}}
    end
  end
  ...
  defp extract_agent_options(section) do
    %{}
    |> put_if_present(:kind, normalize_agent_kind(scalar_string_value(Map.get(section, "kind"))))
  ...
  defp normalize_agent_kind(kind) when is_binary(kind) do
    kind
    |> String.trim()
    |> String.downcase()
    |> case do
